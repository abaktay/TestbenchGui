...
ThrottlePacket::ThrottlePacket(uint16_t value) : arm_status(0), throttle_value(value), checksum(0) {
    header_cksum = header[0] ^ header[1];
}

void ThrottlePacket::update(uint16_t new_value) {
